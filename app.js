...
const CONFIG = {
  ...
  APPS_SCRIPT_URL: "https://script.google.com/macros/s/AKfycbzobvFhSctL0LUjKGxQkHH2eU93GwAcET9MsquxG1PB_AnfiEFdUrv0UPJ8naItDnwLKQ/exec",
  SESSION_HOURS: 8,
};

const demoUsers = [
  { userId: "admin", password: "diwali2026", name: "Festival Admin", role: "admin" },
  { userId: "ops-east", password: "diwali2026", name: "Operations East", role: "user" },
  { userId: "hr-01", password: "diwali2026", name: "People & Culture", role: "user" },
];
const demoTypes = ["Kaju Katli", "Motichoor Ladoo", "Soan Papdi", "Rasgulla", "Gulab Jamun"];
const demoEntries = [
  { id: "DS-2401", person: "Ananya Sharma", type: "Kaju Katli", count: 40, remarks: "VIP visitor kits", userId: "ops-east", userName: "Operations East", timestamp: "Today · 10:42 AM" },
  { id: "DS-2398", person: "Rohit Menon", type: "Motichoor Ladoo", count: 24, remarks: "Front desk team", userId: "hr-01", userName: "People & Culture", timestamp: "Today · 09:18 AM" },
  { id: "DS-2396", person: "Ishita Kapoor", type: "Soan Papdi", count: 18, remarks: "Dispatch with gifts", userId: "sales-north", userName: "Sales North", timestamp: "Yesterday · 04:06 PM" },
  { id: "DS-2392", person: "Vivek Rao", type: "Gulab Jamun", count: 32, remarks: "Night shift", userId: "facilities", userName: "Facilities", timestamp: "Yesterday · 11:31 AM" },
];

let session = loadSession();
let entries = [...demoEntries];
let sweetTypes = [...demoTypes];
let users = demoUsers.map(({ password, ...user }) => ({ ...user, lastActive: "Preview mode" }));
...
async function api(action, payload = {}) {
  if (!isLive()) throw new Error("Preview mode");
...
async function login(event) {
  ...
  try {
    ...
    const demo = demoUsers.find(user => user.userId === userId && user.password === password);
    if (!demo) { showError("Invalid credentials. Try admin / diwali2026 in preview mode."); return; }
    saveSession({ userId: demo.userId, name: demo.name, role: demo.role, expiresAt: Date.now() + CONFIG.SESSION_HOURS * 3600000 });
  }
...
function openApp() {
  ...
  $("settingsNav").classList.toggle("hidden", session.role !== "admin"); $("newEntryNav").classList.toggle("hidden", session.role === "admin"); $("setupBanner").classList.toggle("hidden", isLive());
...
async function loadData() {
  ...
  } catch { /* Demo state remains available until CONFIG is connected. */ }
}
...
async function createUser(event) { event.preventDefault(); const userId = $("newUserId").value.trim(), name = $("newUserName").value.trim(), password = $("newUserPassword").value; if (!userId || !name || password.length < 6) return showToast("Enter valid user details and a 6+ character password."); try { await api("createUser", { userId, name, password }); } catch {} users.unshift({ userId, name, role: "user", lastActive: "Just created" }); showToast("User access created."); renderSettings(); }
